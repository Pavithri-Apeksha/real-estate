  ...
  return (
    <div className='absolute top-0 left-0 w-full z-10'>
      <div className='container mx-auto flex justify-between items-center py-4 px-6 md:px-20 lg:px-32 bg-transparent'>
        ...
        <img onClick={()=>setShowMobileMenu(true)} src={menu_icon} className='md:hidden w-7 cursor-pointer'/>
      </div>
      {/*mobile menu*/}
      <div className={`md:hidden ${showMobileMenu ? 'w-full fixed':'h-0 w-0'}  right-0 top-0 bottom-0 overflow-hidden bg-white transition-all`}>
        <div className="flex justify-end p-6 cursor-pointer">
          <img onClick={()=>setShowMobileMenu(false)} src={close_icon} alt="" className="w-6" />
        </div>
